    ...
    >
      ...
      >
        <div
          ...
          style={{
            background:
              'radial-gradient(circle, rgba(20,184,166,0.12) 0%, rgba(6,78,59,0.06) 50%, transparent 75%)',
            filter: 'blur(40px)',
          }}
        />
      </div>
      ...
      <div className="relative z-10 flex-1 flex lg:items-center overflow-hidden">
        <div className="w-full max-w-7xl mx-auto px-6 md:px-12 lg:px-20 py-6 lg:py-4">

          {/* Grid ─────────────────────────────────────────────────── */}
          {/*
            Mobile order:   label(1) → image(2) → text(3) → stats(4)
            Desktop order:  text(1)  → image(2) → stats(3)
          */}
          <div className="grid grid-cols-1 lg:grid-cols-[1.2fr_1fr_1fr] gap-4 lg:gap-6 items-center">
            ...
            {/* ══ COL 2 — DOCTOR IMAGE ═════════════════════════════
                ...
                order-2 on desktop (center column)
            ════════════════════════════════════════════════════════ */}
            <motion.div
              {...fadeIn(0.2)}
              className="order-2 flex justify-center items-center relative"
            >
              {/* Doctor image */}
              <div
                className="relative z-10
                            w-[280px] sm:w-[300px] lg:w-[500px] xl:w-[560px]
                            lg:-mt-8 xl:-mt-12
                            lg:scale-95 lg:translate-y-3
            ...
            ════════════════════════════════════════════════════════ */}
            <div className="order-3 lg:order-1 w-full flex flex-col items-center lg:items-start text-center lg:text-left relative z-10 lg:z-20">

              {/* Desktop-only label (hidden on mobile — shown separately above) */}
              <motion.div
                variants={fadeUp} initial="hidden" animate="show" custom={0.05}
              ...
              >
                ...
                <br />
                <span className="font-normal italic text-white/70" style={{ fontSize: 'clamp(1.4rem, 2.5vw, 2.1rem)' }}>
                  Compassionate Care.
                </span>
              </motion.h1>
              ...
              <motion.p
                ...
                className="font-body text-white/55 text-sm leading-relaxed max-w-sm mb-2"
              >
              ...
              <motion.p
                ...
                className="font-display italic text-amber-400/80 text-sm mb-5"
              >
              ...
              >
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 bg-amber-400 hover:bg-amber-300 text-[#052e2b] font-body font-bold px-6 py-3 rounded-xl text-sm transition-all duration-300 hover:-translate-y-0.5 shadow-lg shadow-amber-400/20 w-full sm:w-auto"
                >
                ...
                <Link
                  href="/services"
                  className="inline-flex items-center justify-center gap-2 border border-white/20 hover:border-white/40 text-white hover:text-amber-300 font-body font-medium px-6 py-3 rounded-xl text-sm transition-all duration-300 hover:-translate-y-0.5 w-full sm:w-auto"
                >
                  View Specialities
            ...
            {/* ══ COL 3 — STATS PANEL ══════════════════════════════
                order-4 on mobile (below text)
                order-3 on desktop (right column)
            ════════════════════════════════════════════════════════ */}
            <motion.div
              {...fadeIn(0.35)}
              className="order-4 lg:order-3 grid grid-cols-2 lg:grid-cols-1 gap-3 lg:gap-0"
            >
              {stats.map(({ icon: Icon, value, label }, i) => (
                <motion.div
                  key={label}
                  variants={fadeUp} initial="hidden" animate="show" custom={0.45 + i * 0.1}
                  className="flex items-center gap-3 py-0 lg:py-5 lg:border-b lg:border-white/8 last:border-0"
                >
                  <div className="flex-shrink-0 w-9 h-9 rounded-lg border border-white/10 bg-white/5 flex items-center justify-center">
                    <Icon className="w-4 h-4 text-teal-400/80" />
                  </div>
                  <div>
                    <p className="font-display text-amber-400 font-bold leading-none"
                       style={{ fontSize: 'clamp(1.25rem, 2vw, 1.75rem)' }}>
                      {value}
                    </p>
                    <p className="font-body text-white/50 text-xs mt-0.5 leading-snug">{label}</p>
                  </div>
                </motion.div>
              ))}
            </motion.div>
